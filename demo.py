...
def main(sess, age, gender, train_mode, images_pl):
    ...
    while True:
        ...
        if len(detected) > 0:
            ...
            ages, genders = sess.run([age, gender], feed_dict={
                                     images_pl: faces, train_mode: False})

        # draw results
        for i, d in enumerate(detected):
            label = "{}, {}".format(
                int(ages[i]), "FEMALE" if genders[i] == 0 else "MALE")
...
def load_network(model_path):
    ...
    images_pl = tf.placeholder(
        tf.float32, shape=[None, 160, 160, 3], name='input_image')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", "--M",
                        default="./models", type=str, help="Model Path")
    args = parser.parse_args()
